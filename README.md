...
- [ ] Разделить настройки на тест и прод среды
...
# Тестирование 
Важно! Для тестирования используется библиотека 
env 
...
git clone git@github.com:belyashnikovatn/Avito-shop.git
```
Убедитесь, что Docker запущен, файл `.env` создан и заполнен по примеру. В корне проекта выполните команду:
```bash
