...
- В задании указано реализовать эндпоинт `/api/buy/{item}` через метод `get`. Но мною было принято решение использовать `post` метод. Это не влияет на логику сервиса, но, на мой взгляд, больше соответствует приницпам REST. 
...
Для тестирования используется библиотека pytest. Для этого необходимо её установить а также раскоментировать в файле `.env` строку `DEBUG=true`.
