...
- В задании указано реализовать эндпоинт `/api/buy/{item}` через метод `get`. Но мною было принято решение использовать `post` метод. Это не влияет на логику сервиса, но, на мой взгляд, больше соответствует принципам REST. 
- Для проекта используются переменные окружения. Обычно предоставляют файл `.env_example` в качестве примера для заполнения файла `.env`. Но поскольку проект реализован в качестве тестового задания, `.env` лежит в открытом доступе в целях быстрого запуска. 
...
[Посмотреть на примеры работы сервиса](#примеры-работы-сервиса)
...
Docker, Gunicorn, nginx, Postgres (для продакшена)  
...
## Тестирование 
В проекте реализованы unit и e2e-тесты. Тесты находятся в папке `avito_shop/api/pytest-tests`. 
Для тестирования используется библиотека pytest. Для этого необходимо её установить и ещё раскомментировать в файле `.env` строку `DEBUG=true`.

Чтобы установить библиотеки и запустить тесты, выполните следующие команды:
```bash
python -m venv venv
...
- покупка товара
    - не авторизованный не может купить товар
